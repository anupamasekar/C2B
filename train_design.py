...
# parser.add_argument('--ckpt_from', type=str, default=None, help='load ckpt from another expt')
...
## initialize nets
c2b = C2B(block_size=args.blocksize, sub_frames=args.subframes, mask='opt', 
            two_bucket=args.two_bucket, trainable=True).cuda()
invNet = ShiftVarConv2D(out_channels=64, block_size=args.blocksize, two_bucket=args.two_bucket).cuda()
uNet = UNet(in_channel=64, out_channel=args.subframes, instance_norm=False).cuda()

## optimizer
optimizer_code = torch.optim.Adam(list(c2b.parameters()), lr=args.lr, weight_decay=1e-5)
optimizer_net = torch.optim.Adam(list(invNet.parameters())+list(uNet.parameters()),
                                 lr=args.lr, weight_decay=1e-5)
scheduler_code = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_code, mode='min', factor=0.9, 
                                                            patience=5, min_lr=1e-6, verbose=True)
scheduler_net = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_net, mode='min', factor=0.9, 
                                                            patience=5, min_lr=1e-6, verbose=True)
...
if args.ckpt is None:
    ...
    uNet.train()
    optimizer_code.load_state_dict(ckpt['opt_code_state_dict'])
    optimizer_net.load_state_dict(ckpt['opt_net_state_dict'])
    start_epoch = ckpt['epoch'] + 1
    logging.info('Loaded checkpoint from epoch %d'%(start_epoch-1))
torch.save(c2b.code, os.path.join(save_path, 'model', 'mask_init.pth'))
...
for i in range(start_epoch, start_epoch+num_epochs):
    ...
    if (i+1)%10 == 0:
        c2b.update_mask()
        print('mask updated', c2b.code[0,:,0,0].data)
        torch.save(c2b.code, os.path.join(save_path, 'model', 'mask_%.6d.pth'%i))
    ...
    if ((i+1) % 2 == 0) or ((i+1) == (start_epoch+num_epochs)):        
        ...
        if (i+1)%10 == 0:
            scheduler_code.step(val_loss_sum)
        else:
            scheduler_net.step(val_loss_sum)
        invNet.train()
        uNet.train()
        c2b.train()
    ...
    if ((i+1) % 10 == 0) or ((i+1) == (start_epoch+num_epochs)):
        utils.save_checkpoint(state={'epoch': i, 
                                    ...
                                    'opt_code_state_dict': optimizer_code.state_dict(),
                                    'opt_net_state_dict': optimizer_net.state_dict()},
