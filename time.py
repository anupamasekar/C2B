...
for i in range(start_epoch, start_epoch+num_epochs):
    ...
    for gt_vid in training_generator:   
        ...
        # ssim_sum += utils.compute_ssim(highRes_vid, gt_vid).item()
    ...
    logging.info('Finished epoch %4d with loss: %.4f psnr: %.4f ssim: %.4f'
                %(i, loss_sum/train_iter, psnr_sum/(len(training_set)*input_params['num_patches']), 
                    ssim_sum/(len(training_set)*input_params['num_patches'])))
...
#     logger.scalar_summary(tag='training/l1_loss', value=l1_loss_sum/train_iter, step=i)
...
#     logger.scalar_summary(tag='training/ssim', value=ssim_sum/(len(training_set)*input_params['num_patches']), step=i)
#     logging.info('Dumped tensorboard summaries for epoch {}'.format(i))
...
#                 ## loss
#                 l1_loss = l1(highRes_vid, gt_vid)
#                 tv_loss = utils.gradx(highRes_vid).abs().mean() + utils.grady(highRes_vid).abs().mean()

#                 val_loss_sum += (l1_loss + 0.1*tv_loss).item()
