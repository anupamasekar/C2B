...
for i in range(start_epoch, start_epoch+num_epochs):
    ...
    for gt_vid in training_generator:   
        ...
        interm_loss = utils.weighted_L2loss(b1_est, b1)
        interm_loss_sum += interm_loss.item()

        final_loss = utils.weighted_L2loss(highres_vid, gt_vid)
        ...
        loss = final_loss + interm_loss + 0.1*tv_loss
        loss_sum += loss.item()
        ...
        if train_iter % 200 == 0:
            ...
            for frame in range(gt_np.shape[0]):
                ...
                utils.save_image(highres_np[frame,...], 
                                os.path.join(save_path, 'images', 'highres_%.3d_%.5d_%.2d.png'%(i, train_iter, frame)))
